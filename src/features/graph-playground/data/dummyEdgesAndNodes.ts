import { Edge } from 'reactflow';

import { IResourceNode } from '@/features/graph-playground/components/flow-graph/nodes/ResourceNode';
import { IServiceNode } from '@/features/graph-playground/components/flow-graph/nodes/ServiceNode';

export const EDGES: Edge<any>[] = [
  {
    id: 'e0-0',
    source: '0',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e0-1',
    source: '0',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e0-2',
    source: '0',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e0-3',
    source: '0',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e0-4',
    source: '0',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e0-5',
    source: '0',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e0-6',
    source: '0',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e0-7',
    source: '0',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e1-0',
    source: '1',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e1-1',
    source: '1',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e1-2',
    source: '1',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e1-3',
    source: '1',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e2-0',
    source: '2',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e2-1',
    source: '2',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e2-2',
    source: '2',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e2-3',
    source: '2',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e2-4',
    source: '2',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e2-5',
    source: '2',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e2-6',
    source: '2',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e2-7',
    source: '2',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e3-0',
    source: '3',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e3-1',
    source: '3',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e3-2',
    source: '3',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e3-3',
    source: '3',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e3-4',
    source: '3',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e3-5',
    source: '3',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e3-6',
    source: '3',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e3-7',
    source: '3',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e3-8',
    source: '3',
    target: '8',
    type: 'straight',
  },
  {
    id: 'e4-0',
    source: '4',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e4-1',
    source: '4',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e4-2',
    source: '4',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e4-3',
    source: '4',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e4-4',
    source: '4',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e4-5',
    source: '4',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e4-6',
    source: '4',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e4-7',
    source: '4',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e4-8',
    source: '4',
    target: '8',
    type: 'straight',
  },
  {
    id: 'e5-0',
    source: '5',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e5-1',
    source: '5',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e5-2',
    source: '5',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e5-3',
    source: '5',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e5-4',
    source: '5',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e5-5',
    source: '5',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e5-6',
    source: '5',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e6-0',
    source: '6',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e6-1',
    source: '6',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e6-2',
    source: '6',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e6-3',
    source: '6',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e7-0',
    source: '7',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e7-1',
    source: '7',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e7-2',
    source: '7',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e7-3',
    source: '7',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e8-0',
    source: '8',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e8-1',
    source: '8',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e8-2',
    source: '8',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e8-3',
    source: '8',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e9-0',
    source: '9',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e9-1',
    source: '9',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e9-2',
    source: '9',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e9-3',
    source: '9',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e10-0',
    source: '10',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e10-1',
    source: '10',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e10-2',
    source: '10',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e10-3',
    source: '10',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e10-4',
    source: '10',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e10-5',
    source: '10',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e10-6',
    source: '10',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e10-7',
    source: '10',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e10-8',
    source: '10',
    target: '8',
    type: 'straight',
  },
  {
    id: 'e10-9',
    source: '10',
    target: '9',
    type: 'straight',
  },
  {
    id: 'e11-0',
    source: '11',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e11-1',
    source: '11',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e11-2',
    source: '11',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e11-3',
    source: '11',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e12-0',
    source: '12',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e12-1',
    source: '12',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e12-2',
    source: '12',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e12-3',
    source: '12',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e12-4',
    source: '12',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e12-5',
    source: '12',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e13-0',
    source: '13',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e13-1',
    source: '13',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e13-2',
    source: '13',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e13-3',
    source: '13',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e13-4',
    source: '13',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e13-5',
    source: '13',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e13-6',
    source: '13',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e13-7',
    source: '13',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e13-8',
    source: '13',
    target: '8',
    type: 'straight',
  },
  {
    id: 'e13-9',
    source: '13',
    target: '9',
    type: 'straight',
  },
  {
    id: 'e14-0',
    source: '14',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e14-1',
    source: '14',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e14-2',
    source: '14',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e14-3',
    source: '14',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e15-0',
    source: '15',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e15-1',
    source: '15',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e15-2',
    source: '15',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e15-3',
    source: '15',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e15-4',
    source: '15',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e15-5',
    source: '15',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e15-6',
    source: '15',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e15-7',
    source: '15',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e15-8',
    source: '15',
    target: '8',
    type: 'straight',
  },
  {
    id: 'e15-9',
    source: '15',
    target: '9',
    type: 'straight',
  },
  {
    id: 'e16-0',
    source: '16',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e16-1',
    source: '16',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e16-2',
    source: '16',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e16-3',
    source: '16',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e17-0',
    source: '17',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e17-1',
    source: '17',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e17-2',
    source: '17',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e17-3',
    source: '17',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e17-4',
    source: '17',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e17-5',
    source: '17',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e17-6',
    source: '17',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e18-0',
    source: '18',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e18-1',
    source: '18',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e18-2',
    source: '18',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e18-3',
    source: '18',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e18-4',
    source: '18',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e18-5',
    source: '18',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e18-6',
    source: '18',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e19-0',
    source: '19',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e19-1',
    source: '19',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e19-2',
    source: '19',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e19-3',
    source: '19',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e19-4',
    source: '19',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e19-5',
    source: '19',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e19-6',
    source: '19',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e19-7',
    source: '19',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e20-0',
    source: '20',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e20-1',
    source: '20',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e20-2',
    source: '20',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e20-3',
    source: '20',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e21-0',
    source: '21',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e21-1',
    source: '21',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e21-2',
    source: '21',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e21-3',
    source: '21',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e21-4',
    source: '21',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e21-5',
    source: '21',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e21-6',
    source: '21',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e21-7',
    source: '21',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e22-0',
    source: '22',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e22-1',
    source: '22',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e22-2',
    source: '22',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e22-3',
    source: '22',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e23-0',
    source: '23',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e23-1',
    source: '23',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e23-2',
    source: '23',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e23-3',
    source: '23',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e23-4',
    source: '23',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e23-5',
    source: '23',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e23-6',
    source: '23',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e24-0',
    source: '24',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e24-1',
    source: '24',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e24-2',
    source: '24',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e24-3',
    source: '24',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e25-0',
    source: '25',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e25-1',
    source: '25',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e25-2',
    source: '25',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e25-3',
    source: '25',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e25-4',
    source: '25',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e26-0',
    source: '26',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e26-1',
    source: '26',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e26-2',
    source: '26',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e26-3',
    source: '26',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e26-4',
    source: '26',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e26-5',
    source: '26',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e28-0',
    source: '28',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e28-1',
    source: '28',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e28-2',
    source: '28',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e28-3',
    source: '28',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e28-4',
    source: '28',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e28-5',
    source: '28',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e29-0',
    source: '29',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e29-1',
    source: '29',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e29-2',
    source: '29',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e30-0',
    source: '30',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e30-1',
    source: '30',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e31-0',
    source: '31',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e32-0',
    source: '32',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e32-1',
    source: '32',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e32-2',
    source: '32',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e32-3',
    source: '32',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e32-4',
    source: '32',
    target: '4',
    type: 'straight',
  },
  {
    id: 'e32-5',
    source: '32',
    target: '5',
    type: 'straight',
  },
  {
    id: 'e32-6',
    source: '32',
    target: '6',
    type: 'straight',
  },
  {
    id: 'e32-7',
    source: '32',
    target: '7',
    type: 'straight',
  },
  {
    id: 'e32-8',
    source: '32',
    target: '8',
    type: 'straight',
  },
  {
    id: 'e33-0',
    source: '33',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e33-1',
    source: '33',
    target: '1',
    type: 'straight',
  },
  {
    id: 'e33-2',
    source: '33',
    target: '2',
    type: 'straight',
  },
  {
    id: 'e33-3',
    source: '33',
    target: '3',
    type: 'straight',
  },
  {
    id: 'e34-0',
    source: '34',
    target: '0',
    type: 'straight',
  },
  {
    id: 'e34-1',
    source: '34',
    target: '1',
    type: 'straight',
  },
];

export const NODES: (IServiceNode | IResourceNode)[] = [
  {
    id: '0',
    data: {
      label: 'microchip card service name, that is super duper long!!',
      resourceType: 'rds',
      scorecardLevel: 'BRONZE',
      sourceType: 'YAML',
    },
    type: 'service',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '1',
    data: {
      label: 'alarm bandwidth',
      resourceType: 'kafka-topic',
      scorecardLevel: 'SILVER',
      sourceType: 'YAML',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '2',
    data: {
      label: 'panel firewall',
      resourceType: 'squid-proxy',
      scorecardLevel: 'SILVER',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '3',
    data: {
      label: 'bandwidth monitor',
      resourceType: 's3',
      scorecardLevel: 'SILVER',
      sourceType: 'YAML',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '4',
    data: {
      label: 'bus system',
      resourceType: 'rds',
      scorecardLevel: 'GOLD',
      sourceType: 'AWS',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '5',
    data: {
      label: 'bandwidth system',
      resourceType: 'squid-proxy',
      scorecardLevel: 'GOLD',
      sourceType: 'APM',
    },
    type: 'service',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '6',
    data: {
      label: 'pixel transmitter',
      resourceType: 'kafka-topic',
      scorecardLevel: 'GOLD',
      sourceType: 'APM',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '7',
    data: {
      label: 'matrix panel',
      resourceType: 'kafka-topic',
      scorecardLevel: 'SILVER',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '8',
    data: {
      label: 'microchip firewall',
      resourceType: 'rds',
      scorecardLevel: 'SILVER',
      sourceType: 'APM',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '9',
    data: {
      label: 'monitor protocol',
      resourceType: 'rds',
      scorecardLevel: 'SILVER',
      sourceType: 'YAML',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '10',
    data: {
      label: 'application hard drive',
      resourceType: 's3',
      scorecardLevel: 'BRONZE',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '11',
    data: {
      label: 'array monitor',
      resourceType: 'squid-proxy',
      scorecardLevel: 'BRONZE',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '12',
    data: {
      label: 'interface circuit',
      resourceType: 's3',
      scorecardLevel: 'BRONZE',
      sourceType: 'APM',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '13',
    data: {
      label: 'driver matrix',
      resourceType: 'rds',
      scorecardLevel: 'GOLD',
      sourceType: 'AWS',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '14',
    data: {
      label: 'feed capacitor',
      resourceType: 'squid-proxy',
      scorecardLevel: 'SILVER',
      sourceType: 'AWS',
    },
    type: 'service',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '15',
    data: {
      label: 'hard drive protocol',
      resourceType: 'rds',
      scorecardLevel: 'BRONZE',
      sourceType: 'YAML',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '16',
    data: {
      label: 'application matrix',
      resourceType: 's3',
      scorecardLevel: 'SILVER',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '17',
    data: {
      label: 'feed matrix',
      resourceType: 'kafka-topic',
      scorecardLevel: 'BRONZE',
      sourceType: 'AWS',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '18',
    data: {
      label: 'interface panel',
      resourceType: 'kafka-topic',
      scorecardLevel: 'SILVER',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '19',
    data: {
      label: 'monitor feed',
      resourceType: 's3',
      scorecardLevel: 'BRONZE',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '20',
    data: {
      label: 'program matrix',
      resourceType: 's3',
      scorecardLevel: 'SILVER',
      sourceType: 'APM',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '21',
    data: {
      label: 'circuit microchip',
      resourceType: 's3',
      scorecardLevel: 'SILVER',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '22',
    data: {
      label: 'matrix driver',
      resourceType: 'squid-proxy',
      scorecardLevel: 'SILVER',
      sourceType: 'YAML',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '23',
    data: {
      label: 'protocol bus',
      resourceType: 'squid-proxy',
      scorecardLevel: 'GOLD',
      sourceType: 'YAML',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '24',
    data: {
      label: 'sensor pixel',
      resourceType: 's3',
      scorecardLevel: 'SILVER',
      sourceType: 'AWS',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '25',
    data: {
      label: 'hard drive matrix',
      resourceType: 'rds',
      scorecardLevel: 'GOLD',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '26',
    data: {
      label: 'alarm alarm',
      resourceType: 's3',
      scorecardLevel: 'SILVER',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '28',
    data: {
      label: 'card capacitor',
      resourceType: 'kafka-topic',
      scorecardLevel: 'BRONZE',
      sourceType: 'AWS',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '29',
    data: {
      label: 'panel system',
      resourceType: 'rds',
      scorecardLevel: 'SILVER',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '30',
    data: {
      label: 'program microchip',
      resourceType: 's3',
      scorecardLevel: 'SILVER',
      sourceType: 'API',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '31',
    data: {
      label: 'circuit capacitor',
      resourceType: 's3',
      scorecardLevel: 'BRONZE',
      sourceType: 'APM',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '32',
    data: {
      label: 'circuit microchip',
      resourceType: 'rds',
      scorecardLevel: 'GOLD',
      sourceType: 'YAML',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '33',
    data: {
      label: 'pixel monitor',
      resourceType: 'rds',
      scorecardLevel: 'BRONZE',
      sourceType: 'APM',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
  {
    id: '34',
    data: {
      label: 'pixel circuit',
      resourceType: 's3',
      scorecardLevel: 'GOLD',
      sourceType: 'AWS',
    },
    type: 'resource',
    position: {
      x: 0,
      y: 0,
    },
  },
];
